...
void ransac(vector<KeyPoint> &kps_obj, vector<KeyPoint> &kps_target, vector<Point2d> &pts_obj, vector<Point2d> &pts_target, vector<vector<DMatch>> &good_matches, Mat &homography){
	srand(time(NULL)) ;
			...
			for(int j=0; j<good_matches[rndpts[1]].size(); j++){
				...
				for(int k=0; k<good_matches[rndpts[2]].size(); k++){
					...
					for(int m=0; m<good_matches[rndpts[3]].size(); m++){
						...
						//////////////////////////////////
						double error ;
						int num_inliers = inliers(pts_obj, pts_target, good_matches, transform_mat, 60.0, error) ;
						if(num_inliers > good_matches.size()*0.6){
	...
	cout << good_matches.size() << "\t" << max_inliers << endl ;
	homography = candidates[max_index] ;
}
...
void featureDetection(string obj, string target){
	Mat img_obj = imread("object/c0.jpg") ;
	Mat img_target = imread("target/target1.jpg") ;
	...
	knn_match(100, descriptor_obj, descriptor_target, matches, good_matches) ;
	cout << "matches: " << matches.size() << endl ;
	cout << "good matches: " << good_matches.size() << endl ; ;

	Mat homography ;
	ransac_debug(kps_obj, kps_target, pts_obj, pts_target, good_matches, homography, img_obj, img_target) ;
	//ransac(kps_obj, kps_target, pts_obj, pts_target, good_matches, homography) ;
...
int main(){
	featureDetection("a1.jpg", "target1.jpg") ;
	return 0 ;
}